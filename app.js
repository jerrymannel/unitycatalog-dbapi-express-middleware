...
utils.init(logger);
const apiClient = require('./api.utils');
...
function UnityCatalogCRUD(options) {
	...
	this.apiOptions = options.options;
}

function mapResultToColumns(serverResponse) {
	const columns = serverResponse.manifest.schema.columns;
	let data = [];
	if (!serverResponse.result.data_array) return data;
	serverResponse.result.data_array.forEach(row => {
		let obj = {};
...
UnityCatalogCRUD.prototype.list = async function (options) {
	try {
		...
		const result = await apiClient.post(sql, this.apiOptions, logger);
		logger.debug(`list(): List query successful`);
		// return result;
		return mapResultToColumns(result);
	} catch (err) {
...
UnityCatalogCRUD.prototype.fetchOne = async function (value, options) {
	try {
		...
		return mapResultToColumns(result)[0];
	} catch (err) {
...
UnityCatalogCRUD.prototype.count = async function (options) {
	try {
		...
		if (!result.result.data_array || result.result.data_array.length === 0) return { count: 0 }
		return { count: parseInt(result.result.data_array[0][0]) };
	} catch (err) {
